    ...
    addItemToScene: (item, quantity = 1) => {
        for (let i = 0; i < quantity; i++) {
            ...
            const existingBoxes = itemsInScene.map(it => {
                const pos = new THREE.Vector3(...it.position);
            ...
            // 演算法：嘗試在不同位置放置
            // 1. 先嘗試放在地板上
            for (let y = newItemDims.h / 2; y < spaceDims.h; y += 0.05) {
                for (let x = -spaceDims.w / 2 + newItemDims.w / 2; x <= spaceDims.w / 2 - newItemDims.w / 2; x += 0.05) {
                    for (let z = -spaceDims.d / 2 + newItemDims.d / 2; z <= spaceDims.d / 2 - newItemDims.d / 2; z += 0.05) {
                        ...
                        if (!hasCollision && y < lowestY) {
                            lowestY = y;
                            bestPosition = [newPos.x, newPos.y, newPos.z];
                        }
                    }
                }
                if (bestPosition) break; // 如果在地板層找到位置，就不再往上找
            }

            // 如果在地板上找不到位置，則嘗試堆疊在其他物品上
            if (!bestPosition) {
                // ... (更複雜的堆疊演算法可以後續加入)
            }
